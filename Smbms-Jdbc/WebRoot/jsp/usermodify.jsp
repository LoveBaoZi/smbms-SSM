...
<div class="right">
	...
	<div class="providerAdd">
		<form id="userForm" name="userForm" method="post"
			action="${pageContext.request.contextPath }/Sys/user/updateUserSave">
			...
			<div>
				...
				<input type="hidden" value="${user.userRole}" id="rid" /> <select
					...
					<option value="1"
						<c:if test="${user.userRole == 1 } ">selected</c:if>>系统管理员</option>
					<option value="2"
						<c:if test="${user.userRole == 2 }">selected</c:if>>经理</option>
					<option value="3"
						<c:if test="${user.userRole == 3 }">selected</c:if>>普通用户</option>
