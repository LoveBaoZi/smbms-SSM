...
$(function() {
	...
	$(".viewUser").on("click", function() {
		...
		// window.location.href = path + "" + obj.attr("userid");
		$.ajax({
			type : "GET",
			url : path + "/Sys/user/userViewAjax",
			data : {
			...
			success : function(result) {
				...
				$("#v_userBirthday").val(result.birthday);
				$("#v_userAddress").val(result.address);
				$("#v_userPhone").val(result.phone);
	...
	$(".modifyUser").on(
			...
			function() {
				var obj = $(this);
				window.location.href = path + "/Sys/user/selectUser?uid="
						+ obj.attr("userid");
			});
