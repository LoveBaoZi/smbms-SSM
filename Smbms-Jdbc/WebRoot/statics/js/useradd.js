...
$(function() {
	...
	if (errorInfo.val() == null || errorInfo.val() == "") {
		a_idPicPath.next().html("* 上传大小不能超过 500 K，上传类型必须为： jpg，jpeg，png，pneg");
	} else {
		a_idPicPath.next().html(errorInfo.val());
	}
	
	if (errorinfo_wp.val() == null || errorinfo_wp.val() == "") {
		a_workPicPath.next().html("* 上传大小不能超过 500 K，上传类型必须为： jpg，jpeg，png，pneg");
	} else {
	...
	$.ajax({
		type : "GET",// 请求类型
		url : path + "/Sys/user/findUserRole",// 请求的url
	...
	userCode.bind("blur", function() {
		...
		$.ajax({
			type : "GET",// 请求类型
			url : path + "/Sys/user/userExits",// 请求的url
			data : {
				userCode : userCode.val()
			},// 请求参数
			dataType : "json",// ajax接口（请求url）返回的数据类型
			success : function(data) {// data：返回数据（json对象）
				if (data.userCode == "no") {// 账号已存在，错误提示
					validateTip(userCode.next(), {
						"color" : "red"
